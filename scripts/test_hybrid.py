#!/usr/bin/env python3
"""Fuse PostgreSQL BM25 and Qdrant dense paragraph results with RRF."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fuse BM25 and dense legal-paragraph rankings using RRF.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of fused matches to print (default: %(default)s)",
    )
    parser.add_argument(
        "--bm25-depth",
        type=int,
...
def main(argv: list[str] | None = None) -> int:
    ...
    provider = SentenceTransformerEmbeddingProvider(
        args.model or settings.embedding_model,
    ...
    dense_retriever = DenseParagraphRetriever(paragraph_index, provider)
    ...
    print(
        ...
        f"{diagnostics.bm25_candidates} dense={diagnostics.dense_candidates} "
        f"unique={diagnostics.unique_candidates}; fusion="
        f"{diagnostics.fusion_seconds * 1000:.3f} ms total="
        f"{diagnostics.total_seconds * 1000:.1f} ms"
    )
    if not results:
    ...
    for result in results:
        ...
        print(
            f"{result.rank}. rrf={result.rrf_score:.8f} "
